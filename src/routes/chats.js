...
router.delete('/messages/:messageId', async (req, res) => {
  try {
    ...
    await query('UPDATE messages SET content = $2, attachment_url = NULL, attachment_name = NULL, deleted_at = NOW() WHERE id = $1', [messageId, '']);
    const formatted = await formatMessage(messageId);
    req.app.get('io').to(message.chat_id).emit('message:update', formatted);
    await emitChatRefresh(req.app, message.chat_id);
...
router.post('/:chatId/messages', upload.array('attachment', 10), async (req, res) => {
  try {
    ...
    if (!files.length) {
      ...
      await query(
        `INSERT INTO messages
         (id, chat_id, user_id, content, message_type, attachment_url, attachment_name, reply_to_message_id, album_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [messageId, chatId, req.user.id, content, 'text', null, null, replyToMessageId, null]
      );
      ...
      for (let i = 0; i < files.length; i += 1) {
        ...
        await query(
          `INSERT INTO messages
           (id, chat_id, user_id, content, message_type, attachment_url, attachment_name, reply_to_message_id, album_id)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
          [messageId, chatId, req.user.id, i === 0 ? content : '', 'file', attachmentUrl, attachmentName, i === 0 ? replyToMessageId : null, albumId]
        );
        createdMessages.push(await formatMessage(messageId));
      }
    }
